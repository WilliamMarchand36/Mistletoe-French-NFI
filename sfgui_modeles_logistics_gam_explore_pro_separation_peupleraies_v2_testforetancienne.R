...
data_placette <- read.csv("data_plots_2008_2022_all_v2_foretancienne_precis.csv", header = TRUE)
extra_data <- read.csv("gtot_sfocalc_age1_v2.csv", sep = ";", header = TRUE)

unique(extra_data$plantation[extra_data$gestion == "gere"])
res <- summaryBy(plantation~plantation+propriete,
                 data = extra_data,
                 FUN = length
)
res

data_placette_extra <- left_join(data_placette, extra_data, by = "npp")
nrow(data_placette_extra)
nrow(data_placette_extra[data_placette_extra$gest == "NULL", ])
nrow(data_placette_extra[data_placette_extra$age_1 == "NULL", ])
nrow(data_placette_extra[data_placette_extra$plantation == "NULL", ])
nrow(data_placette_extra[data_placette_extra$propriete == "NULL", ])
nrow(data_placette)
data_placette_extra$age_1[data_placette_extra$age_1 == "NULL"] <- NA
data_placette_extra$gest[data_placette_extra$gest == "NULL"] <- NA
data_placette_extra$gestion[data_placette_extra$gestion == "NULL"] <- NA
data_placette_extra$plantation[data_placette_extra$plantation == "NULL"] <- NA
data_placette_extra$age_1 <- as.numeric(data_placette_extra$age_1)
range(data_placette_extra$age_1, na.rm = TRUE)

data_placette <- data_placette_extra
unique(data_placette$gestion)
...
data_placette_abietis$Nature <- as.factor(data_placette_abietis$Nature)
data_placette_abietis$Nature_recode <- ifelse(data_placette_abietis$Nature != "foret ancienne", "foret recente", "foret ancienne")
...
AIC(test_gam_placettes_abietis) #3057 3055 3048.7 3047.5 3046.4
anova(test_gam_placettes_abietis)

plot(ggpredict(test_gam_placettes_abietis, c("Nature_recode")))
...
write.csv(p.table, "coefs_abiet_npp_new2_nature.csv", row.names = FALSE)

s.table <- as.data.frame(summ_npp_abiet$s.table)
s.table$var <- rownames(s.table)
write.csv(s.table, "coefs_smooth_abiet_npp_new2_nature.csv", row.names = FALSE)
...
saveRDS(data_predict, "predict_env_abietis_new2_nature.RDS")
data_predict <- readRDS("predict_env_abietis_new2_nature.RDS")
...
ggsave("pred_abietis_env_new2_nature.png", width = 16, height = 6, dpi = 300)
ggsave("pred_abietis_env_new2_nature.svg", width = 16, height = 6)
...
predict_abietis_plantation$x <- ifelse(predict_abietis_plantation$x == "non plante", "Absence (not planted)", "Presence (planted)")
...
predict_abietis_div$x <- ifelse(predict_abietis_div$x == 1, "1 predominant species", 
                                ifelse(predict_abietis_div$x == 2, "2 predominant species", 
                                ifelse(predict_abietis_div$x == 3, "3 predominant species", 
                                "4+ predominant species")))
...
predict_abietis_pro$x <- ifelse(predict_abietis_pro$x == "Autre forêt publique", "Other public forest", 
                                ifelse(predict_abietis_pro$x == "Forêt domaniale", "State-owned forest", 
                                "Private forest"))
...
data_placette_austriacum$Nature <- as.factor(data_placette_austriacum$Nature)
data_placette_austriacum$Nature_recode <- ifelse(data_placette_austriacum$Nature != "foret ancienne", "foret recente", "foret ancienne")
...
AIC(test_gam_placettes_austriacum) # 2212.9 2211.5 2209.3 2209.4 2209
...
p.table$var <- rownames(p.table)
write.csv(p.table, "coefs_austr_npp_new2_nature.csv", row.names = FALSE)

s.table <- as.data.frame(summ_npp_austr$s.table)
s.table$var <- rownames(s.table)
write.csv(s.table, "coefs_smooth_austr_npp_new2_nature.csv", row.names = FALSE)
...
saveRDS(data_predict, "predict_env_austriacum_new2_nature.RDS")
data_predict <- readRDS("predict_env_austriacum_new2_nature.RDS")
data_predict$variable_factor <- factor(data_predict$variable,
                                       levels = c(
                                         "hydr_r",
                                         "troph_r",
...
plot(predict_austriacum_plantation)
predict_austriacum_plantation$x <- ifelse(predict_austriacum_plantation$x == "non plante", "Absence (not planted)", "Presence (planted)")
...
predict_austriacum_div$x <- ifelse(predict_austriacum_div$x == 1, "1 predominant species", 
                                ifelse(predict_austriacum_div$x == 2, "2 predominant species", 
                                       ifelse(predict_austriacum_div$x == 3, "3 predominant species", 
                                              "4+ predominant species")))
...
data_placette_album$Nature <- as.factor(data_placette_album$Nature)
data_placette_album$Nature_recode <- ifelse(data_placette_album$Nature != "foret ancienne", "foret recente", "foret ancienne")
...
AIC(test_gam_placettes_album) # 4012.9 4008.1 4006.1 4005.5
...
p.table$var <- rownames(p.table)
write.csv(p.table, "coefs_album_npp_new_autre2_nature.csv", row.names = FALSE)

s.table <- as.data.frame(summ_npp_album$s.table)
s.table$var <- rownames(s.table)
write.csv(s.table, "coefs_smooth_album_npp_new_autre2_nature.csv", row.names = FALSE)
...
list_vars <- names(test_gam_placettes_album$model)[-c(1:4, ncol(test_gam_placettes_album$model))]
...
saveRDS(data_predict, "predict_env_album_new_autre2_nature.RDS")
data_predict <- readRDS("predict_env_album_new_autre2_nature.RDS")
data_predict$variable_factor <- factor(data_predict$variable,
                                       levels = c(
                                         "hydr_r",
                                         "troph_r",
...
ggsave("pred_album_env_new_autre2_nature.png", width = 16, height = 6, dpi = 300)
ggsave("pred_album_env_new_autre2_nature.svg", width = 16, height = 6)

### effect species diversity and plantation ####

predict_album_plantation <- ggpredict(test_gam_placettes_album, c('plantation_factor'))
plot(predict_album_plantation)
predict_album_plantation$x <- ifelse(predict_album_plantation$x == "non plante", "Absence (not planted)", "Presence (planted)")
...
predict_album_pro$x <- ifelse(predict_album_pro$x == "Autre forêt publique", "Other public forest", 
                                ifelse(predict_album_pro$x == "Forêt domaniale", "State-owned forest", 
                                       "Private forest"))
...
data_placette_album_peupleraie$Nature <- as.factor(data_placette_album_peupleraie$Nature)
data_placette_album_peupleraie$Nature_recode <- ifelse(data_placette_album_peupleraie$Nature != "foret ancienne", "foret recente", "foret ancienne")
...
AIC(test_gam_placettes_album_peupleraie) # 802.8 800.8 799 798 797 798
anova(test_gam_placettes_album_peupleraie)

plot(ggpredict(test_gam_placettes_album_peupleraie, c("Nature_recode")))
...
p.table$var <- rownames(p.table)
write.csv(p.table, "coefs_album_peupleraie_npp_new_peupleraie2_nature.csv", row.names = FALSE)

s.table <- as.data.frame(summ_npp_album_peupleraie$s.table)
s.table$var <- rownames(s.table)
write.csv(s.table, "coefs_smooth_album_peupleraie_npp_new_peupleraie2_nature.csv", row.names = FALSE)
...
list_vars <- names(test_gam_placettes_album_peupleraie$model)[-c(1:2, ncol(test_gam_placettes_album_peupleraie$model))]
...
saveRDS(data_predict, "predict_env_album_peupleraie_new_peupleraie2_nature.RDS")
data_predict <- readRDS("predict_env_album_peupleraie_new_peupleraie2_nature.RDS")
data_predict$variable_factor <- factor(data_predict$variable,
                                       levels = c(
                                         "hydr_r",
                                         "troph_r",
...
ggsave("pred_album_peupleraie_env_new_peupleraie2_nature.png", width = 16, height = 6, dpi = 300)
ggsave("pred_album_peupleraie_env_new_peupleraie2_nature.svg", width = 16, height = 6)
...
library(ggpubr)
ggarrange(plot_plante, plot_div, align = "hv")

ggsave("plante_div_paspeupleraies2_nature.png", width = 12, height = 5, dpi = 300)
...
ggsave("propriete_paspeupleraies2_nature.png", width = 6, height = 5, dpi = 300)

library(ggpubr)
ggarrange(plot_plante, plot_div, plot_pro, as_ggplot(legend), align = "hv", nrow = 2, ncol = 2)

ggsave("plante_div_pro_paspeupleraies2_nature.png", width = 12, height = 8, dpi = 300)
ggsave("plante_div_pro_paspeupleraies2_nature.svg", width = 12, height = 8)
...
data_ir5_bio <- left_join(data_ir5_bio, data_placette[, c("npp", "plantation", "gestion", "propriete", "gtot_ha", "u_sfo_calc", "Nature")], by = c("npp"))
...
data_ir5_bio$Nature <- as.factor(data_ir5_bio$Nature)
data_ir5_bio$Nature_recode <- ifelse(data_ir5_bio$Nature != "foret ancienne", "foret recente", "foret ancienne")
